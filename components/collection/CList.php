...
class CList extends CComponent implements IteratorAggregate,ArrayAccess,Countable
{
	...
	public function itemAt($index)
	{
		if(isset($this->_d[$index]))
			...
			throw new CException('List index "{index}" is out of bound.',
				array('{index}'=>$index));
	}
	...
	public function insertAt($index,$item)
	{
		if(!$this->_r)
		{
			...
			else if($index>=0 && $index<$this->_c)
			{
				...
				throw new CException('yii','List index "{index}" is out of bound.',
					array('{index}'=>$index));
		}
		else
			throw new CException('yii','The list is read only.');
	}
	...
	public function copyFrom($data)
	{
		if(is_array($data) || ($data instanceof Traversable))
		{
			...
			throw new CException('List data must be an array or an object implementing Traversable.');
	}
